...
enum class PlayerStatus { ACTIVE, WITHDRAWN };
...
class Player {
 ...
 public:
  ...
  void setState(float new_score, int new_games_played, int new_games_as_white,
                bool had_bye, PlayerStatus new_status);
...
vector<float> getUserResults(const vector<Match>& matches,
                             const vector<Player>& players);
void processResults(vector<Player>& players, const vector<Match>& matches,
...
void saveTournament(const string& filename, const vector<Player>& players,
                    int next_round);
...
int main() {
  ...
  while (true) {
    ...
    if (end_tournament) break;

    vector<Match> matches = generatePairings(players, current_round);

    long active_count = 0;
    for (const auto& p : players)
      if (p.getStatus() == PlayerStatus::ACTIVE) active_count++;
    if (active_count < 2) {
      cout << "Not enough active players to continue. Ending tournament."
           << endl;
      break;
    }
...
Color Player::getLastColor() const {
  return color_history.empty() ? Color::NONE : color_history.back();
}
...
void Player::addOpponent(const string& opponent_id) {
  opponent_ids.push_back(opponent_id);
}
...
bool Player::hasPlayedAgainst(const string& opponent_id) const {
  for (const string& past_id : opponent_ids)
    if (past_id == opponent_id) return true;
  return false;
}
void Player::setState(float new_score, int new_games_played,
                      int new_games_as_white, bool had_bye,
...
void Player::setOpponents(const vector<string>& new_opponents) {
  opponent_ids = new_opponents;
}
void Player::setColorHistory(const vector<Color>& history) {
...
// void withdrawPlayer(vector<Player>& players) { /* ... implementation from
// previous response ... */ }
void withdrawPlayer(vector<Player>& players) {
  string name;
  ...
  if (matches.empty()) {
    ...
    cout << matches[0]->getName() << " has been withdrawn from pairings."
         << endl;
  } else {
    ...
    for (auto* p : matches) {
      if (p->getID() == id_to_withdraw) {
        ...
        cout << p->getName() << " (ID: " << p->getID()
             << ") has been withdrawn." << endl;
        success = true;
        break;
      }
    }
    if (!success) cout << "Invalid ID. No player withdrawn." << endl;
  }
}
...
vector<Match> generatePairings(vector<Player>& players, int round) {
  ...
  for (auto& p : players)
    if (p.getStatus() == PlayerStatus::ACTIVE) active_players.push_back(&p);
  cout << "\nThere are " << active_players.size()
       << " active players for this round." << endl;
  ...
  if (round == 1) {
    ...
    sort(active_players.begin(), active_players.end(),
         [](const Player* a, const Player* b) {
           if (a->getScore() != b->getScore())
             return a->getScore() > b->getScore();
           if (a->getBuchholzScore() != b->getBuchholzScore())
             return a->getBuchholzScore() > b->getBuchholzScore();
  ...
  auto pair_players = [&](Player* player1, Player* player2) {
    ...
    if (diff1 < diff2) {
      ...
      if (player1->getLastColor() == Color::BLACK &&
          player2->getLastColor() != Color::BLACK) {
        p1_gets_white_preference = true;
      } else if (player2->getLastColor() == Color::BLACK &&
                 player1->getLastColor() != Color::BLACK) {
        p1_gets_white_preference = false;
      } else {
        p1_gets_white_preference = true;
      }
  ...
  for (size_t i = 0; i < active_players.size(); ++i) {
    ...
    for (size_t j = i + 1; j < active_players.size(); ++j) {
      if (is_paired[j]) continue;
      cout << "Forcing a rematch to complete pairings: "
           << active_players[i]->getName() << " vs "
           << active_players[j]->getName() << endl;
      pair_players(active_players[i], active_players[j]);
      is_paired[i] = true;
      is_paired[j] = true;
      break;
...
vector<float> getUserResults(const vector<Match>& matches,
  ...
  for (const auto& p : players) name_map[p.getID()] = p.getName();
  vector<float> results(matches.size(), -1.0f);
  int results_entered = 0;
  while (true) {
    cout << "\n-- Current Pairings --" << endl;
    for (size_t i = 0; i < matches.size(); ++i) {
      cout << i + 1 << ". " << name_map.at(matches[i].white_player_id)
           << " (W) vs. " << name_map.at(matches[i].black_player_id) << " (B)";
      if (results[i] != -1.0f) {
        cout << " [Result: ";
        if (results[i] == 1.0f)
          cout << "White Wins";
    ...
    if (results_entered == matches.size()) {
      ...
      // If they type 'n', the loop continues, allowing them to overwrite a
      // mistake
    }
    ...
    if (!is_new_result) {
      ...
      if (overwrite_choice != 'y' && overwrite_choice != 'Y') continue;
    }
    float result;
    while (true) {
    ...
    if (is_new_result) results_entered++;
  }
...
void processResults(vector<Player>& players, const vector<Match>& matches,
  ...
  for (size_t i = 0; i < matches.size(); ++i) {
    ...
    for (auto& p : players) {
      if (p.getID() == white_id) white_player = &p;
      if (p.getID() == black_id) black_player = &p;
    }
...
void calculateBuchholzScores(vector<Player>& players) {
  ...
  for (const auto& p : players) score_map[p.getID()] = p.getScore();
  for (auto& p : players) {
    ...
    if (opponent_scores.size() > 2) {
      ...
      for (float score : opponent_scores) median_buchholz_total += score;
    }
...
void printStandings(vector<Player>& players, int round, ostream& out) {
  ...
  out << left << setw(5) << "Rank" << setw(20) << "Name" << setw(10) << "Score"
      << setw(12) << "Buchholz" << setw(12) << "Status" << setw(15) << "ID"
      << endl;
  out << "---------------------------------------------------------------------"
         "-----------"
      << endl;
  ...
  for (size_t i = 0; i < players.size(); ++i) {
    ...
    out << left << setw(5) << to_string(current_rank) + "." << setw(20)
        << players[i].getName() << setw(10) << fixed << setprecision(1)
        << players[i].getScore() << setw(12) << fixed << setprecision(1)
        << players[i].getBuchholzScore();
...
void saveTournament(const string& filename, const vector<Player>& players,
  ...
  for (const auto& p : players) {
    file << p.getID() << "|" << p.getName() << "|" << p.getScore() << "|"
         << p.getGamesPlayed() << "|" << p.getGamesAsWhite() << "|"
         << p.hasReceivedBye() << "|" << static_cast<int>(p.getStatus()) << "|";
    for (Color c : p.getColorHistory()) file << static_cast<int>(c) << " ";
    file << "|";
    for (const string& id : p.getOpponentIDs()) file << id << " ";
    file << "\n";
  }
  file.close();
...
TournamentState loadTournament(const string& filename) {
  ...
  while (getline(file, line)) {
    ...
    int c_int;
    while (color_ss >> c_int) history.push_back(static_cast<Color>(c_int));
    ...
    vector<string> opponent_ids;
    string opp_id;
    while (opponent_ss >> opp_id) opponent_ids.push_back(opp_id);
...
bool loadPlayersFromFile(const string& filename, vector<Player>& players) {
  ...
  file.close();
  cout << "Successfully loaded " << count << " players from " << filename
       << endl;
  return true;
}
